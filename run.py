...
def run_exposure(config: Configuration, run_dir: Path) -> Dict[str, Any]:
    ...
    for step, t in enumerate(config.exposure.readout_times):
        ...
        if detector.image.array is not None:
            np.save(images_dir / f"frame_{step:04d}.npy", detector.image.array)
            meta = {
                "time": t,
                "metadata": detector.image.metadata,
            }
            (images_dir / f"frame_{step:04d}.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
...
def main():
    ...
    manifest = {
        "mode": config.mode,
        "stages": list(config_to_dict(config)["pipeline"].keys()),
    }
