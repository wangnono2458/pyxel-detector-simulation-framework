...
### ✨Improve support for Astropy units

It is now possible to use Astropy `Quantity` data directly in Pyxel's data buckets such as `Photon`, 
`Charge`, `Pixel`, and others.

This improvement enables a more natural and consistent integration of physical units throughout detector modeling. 

Examples:

```python
>>> from astropy.units import Quantity
>>> detector.signal += Quantity(..., unit="uV")
>>> detector.signal = detector.signal + Quantity(..., unit="V")

>>> Quantity(detector.signal)
<Quantity [[ ... ]] V>
```

### ✨ New parameter `data_path` in function `load_image`

The function `pyxel.inputs.load_image` used to load 2D data from files (.fits, .npy, etc...), now accepts
a new optional parameter: `data_path`.

This parameter enbales access:
* specific HDU indices or names for FITS files
* groups for netCDF or HDF5 files
* reference paths for ASDF files
* ...

Feature contributed by Doby Baxter.

### ✨ Channel-specific  `pre_amplification` in `Characteristics`

The `pre_amplification` parameter in the `Characteristics` section can now be defined per readout channel
...
### ⚡️ Speedup for pipelines with multiple readout times

Running a pipeline with multiple readout times is now significantly faster.

### 🧩 Improved `Pixel` handling for volatile and non-volatile effects in `Charge Collection`

During Non-Destructive Readout (NDR), models in the `Charge Collection` group simulate both:
* non-volatile (persistent) effects between readouts (e.g. charge accumulation in a CMOS Pinned PhotoDiode). 
* volatile (transient) effects that reset at each readout (e.g. source-follower noise).

Two new `pixel.non_volatile` and `pixel.volatile` data buckets are introduced to clarify these effects.
The main `pixel` bucket is now **read-only** and combines `pixel.non_volatile` and `pixel.volatile`


Example:
```python

>>> detector.pixel.non_volatile.array = ...  # For non-volatile / persistent effects
>>> detector.pixel.volative_array = ...  # For volatile / transient effects

>>> detector.pixel  # Read-only !
>>> return detector.pixel.non_volatile.array + detector.pixel.volatile.array
```

You can find more information in this issue comment: https://gitlab.com/esa/pyxel/-/issues/956#note_2849620175

Please note that the names `non_volatile` and `volatile` are provisional and may change in a future release.

Feature contributed by Antoine Kaszczyc and Aurélien Jarno from CRAL (Centre de Recherche Astrophysique de Lyon).

### ✨ Support for 2D PSF (Point-Spread Function) with 3D Photon

The `load_psf` model in the `Photon Collection` group now supports using a 2D Point Spread Function (PSF) with a 3D photon
object.

### ⚡️ New model `avalanche_gain` in `Charge Generation` for APD detectors

A new model `avalanche_gain` has been added to the `Charge Generation` group.
It simulates the avalanche multiplication process in the Avalanche Photodiode (APD) detectors.
An excess noise factor can be included to simulate stochastic variations in the gain process.
...
Feature contributed by Jean Le Graët, CEA Paris-Saclay

### Core
* Fix 'FutureWarnings' generated by Xarray.
...
* Implementing `volatile` and `non-volatile` pixels for non-destructive readout mode by 
  Antoine Kaszczyc and Aurélien Jarno from CRAL (Centre de Recherche Astrophysique de Lyon)
  (See [!1120](https://gitlab.com/esa/pyxel/-/merge_requests/1120)).
* Add `capacitance` in Characteristics for CCD, CMOS and APD detectors.
  (See [!1112](https://gitlab.com/esa/pyxel/-/merge_requests/1112)).
...
* Add new model `Avalanche gain` following gamma distribution from Jean Le Graet, CEA-Saclay.
